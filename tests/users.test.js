...
async function loadHandler() {
  vi.resetModules();
  const mod = await import("../netlify/functions/users.js");
  return mod.handler;
}
...
describe("users.js", () => {
  ...
  it("POST returns 405", async () => {
    const handler = await loadHandler();
    expect((await handler(makeEvent({ httpMethod: "POST" }))).statusCode).toBe(405);
  });

  it("returns 503 when FIREBASE_DB_URL is not set", async () => {
    delete process.env.FIREBASE_DB_URL;
    const handler = await loadHandler();
    expect((await handler(makeEvent())).statusCode).toBe(503);
  });
