// Auth router — consolidates login + register + refresh + revoke into one
// Serverless Function to keep us comfortably under Vercel's Hobby-plan
// 12-function cap. Dispatch:
//
//   POST /api/auth                       → login (default, for back-compat)
//   POST /api/auth?action=login          → login
//   POST /api/auth?action=register       → register a new user
//   POST /api/auth?action=refresh        → exchange refresh token
//   POST /api/auth?action=revoke         → revoke refresh token (logout)
//
// vercel.json rewrites also keep /api/register, /api/refresh, /api/revoke
// pointing here so cached JS bundles don't 404 during the rollout window.
...
async function doLogin({ name, hash, legacy }, secret, DB) {
  ...
  // Try the dedicated auth store first.
  ...
  // Fall back to legacy profile list (users predating the auth separation).
  ...
  await fetch(`${DB}/rq/rq-auth-v6.json${fb}`, {
    method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(newAuthList),
  });
...
async function doRegister({ name, hash }, secret, DB) {
  ...
  const wp = await fetch(`${DB}/rq/rq-users-v6.json${fb}`, {
    method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(newProfiles),
  });
  if (!wp.ok) {
...
async function doRevoke({ name, refreshToken }, secret, DB) {
  ...
  try {
    ...
    await fetch(`${DB}/rq/rq-revoked-v1.json${fb}`, {
...
async function doOAuth(body, secret, DB) {
  ...
  // New user — the client must supply a username before we create the account.
  if (!username) {
...
export default async function handler(req, res) {
  ...
  const body = req.body || {};
  const action = String((req.query && req.query.action) || body.action || "login").toLowerCase();
  ...
  // login/register also need hash; refresh/revoke need refreshToken.
  const { name } = body;
  if (!name) return res.status(400).json({ error: "Missing credentials" });

  let result;
  try {
    if (action === "login") {
      if (!body.hash) return res.status(400).json({ error: "Missing credentials" });
      result = await doLogin(body, secret, DB);
    } else if (action === "register") {
      if (!body.hash) return res.status(400).json({ error: "Missing credentials" });
      result = await doRegister(body, secret, DB);
    } else if (action === "refresh") {
      if (!body.refreshToken) return res.status(400).json({ error: "Missing credentials" });
      result = await doRefresh(body, secret, DB);
    } else if (action === "revoke") {
      if (!body.refreshToken) return res.status(400).json({ error: "Missing credentials" });
      result = await doRevoke(body, secret, DB);
    } else {
      return res.status(400).json({ error: "Unknown 'action' (expected login|register|refresh|revoke|oauth)" });
    }
  } catch (e) {
    return res.status(500).json({ error: e.message });
  }
